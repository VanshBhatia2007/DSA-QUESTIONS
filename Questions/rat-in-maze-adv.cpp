...
bool ratinmaze(char maze[][5],int i,int j,int n,int m,int sol[][10]){
    ...
    //check right
    if(j+1<m and maze[i][j+1]!='X' and maze[i+1][j]!='X'){
    ...
    //check down
    if(i+1<n and maze[i+1][j]!='X' and maze[i][j+1]!='X'){
    ...
    //chech left
    if(j-1>=0 and maze[i][j-1]!='X' and maze[i-1][j]!='X'){
        bool left=ratinmaze(maze,i,j-1,n,m,sol);
        if(left) return true;
    }
    //chech upper
    if(i-1>=0 and maze[i-1][j]!='X' and maze[i][j-1]!='X'){
        bool upper=ratinmaze(maze,i-1,j,n,m,sol);
